...
1. [train_esrgan_multi.json]($train_esrgan_multi_json)
...
```c++
{
  "name": "debug_002_RRDB_ESRGAN_multi_x4_DIV2K" // leading 'debug_' enters the 'debug' mode. Please remove it during formal training
  , "use_tb_logger": true // use tensorboard_logger, ref: `https://github.com/xinntao/BasicSR/tree/master/codes/utils`
  , "model":"srragan" // "srgan" |"srragan", model type, ref: `https://github.com/xinntao/BasicSR/blob/master/codes/models/__init__.py`
  , "scale": 4 // scale factor for SR
  , "gpu_ids": [0] // specify GPUs, actually it sets the `CUDA_VISIBLE_DEVICES`
  ...
  , "train": { // training strategies
    ...
    , "weight_decay_G": 0
    , "beta1_G": 0.9
    , "lr_D": {"128": 1e-4, "64": 1e-4, "32": 1e-4} // initialized learning rates for discriminators of different sizes
    , "weight_decay_D": {"128": 0, "64": 0, "32": 0}
    , "beta1_D": {"128": 0.9, "64": 0.9, "32": 0.9}
    , "lr_scheme": "MultiStepLR" // learning rate decay scheme
    , "lr_steps": [50000, 100000, 200000, 300000] // at which steps, decay the learining rate
    , "lr_gamma": 0.5
    , "num_D": {"128": 1, "64": 2, "32": 4} // number of D of each size
    , "sample_strategy": "random" // "random" | "pyramid" | "adaptive", sample strategy for discriminators
    ...
    , "feature_weight": 1
    , "gan_type": "vanilla" // GAN type
    , "gan_weight": {"128": 5e-3, "64": 5e-3, "32": 5e-3}
